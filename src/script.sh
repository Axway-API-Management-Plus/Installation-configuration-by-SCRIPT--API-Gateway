...
./$installationFile --mode unattended --setup_type advanced --licenseFilePath $licenseFilePath --prefix $dir --enable-components $enable_components --disable-components $disable_components --username $username --adminpasswd $adminpasswd --cassandraInstalldir $cassandraInstalldir --cassandraJDK $cassandraJDK --startCassandra $cassandraStart --analyticsLicenseFilePath $analyticsLicenseFilePath --apimgmtLicenseFilePath $apimgmtlicenseFilePath
...
#go to the installation_dir/apigateway/samples/scripts where the run.sh file is present that is used at the time of instance creation and add cassndra node entry in it (usually localhost 9042 is used, if the same machine has the cassandra in it)
        ...
        ./setup-cassandra --seed-ip $Cass_seed --own-ip $Cass_IP --cassandra-config $cassandraInstalldir/cassandra/conf/cassandra.yaml
        cd $cassandraInstalldir/cassandra/bin
        ./cassandra
...
# domain registration taked place here addressing the admin node
        ...
        if [ $admin_capability == 'yes' ]

                then

                #give admin capability to this Nodemanager

                        ./managedomain --add --nm_name $nm_name --anm_host $anmhost --anm_port $anmport --username $username --password $password --is_admin
